...
namespace System.Reactive.Concurrency
{
    ...
    public sealed class CurrentThreadScheduler : LocalScheduler
    {
        ...
        private static ThreadLocal<SchedulerQueue<TimeSpan>> s_threadLocalQueue = new ThreadLocal<SchedulerQueue<TimeSpan>>();

        //[ThreadStatic]
        //private static IStopwatch s_clock;
        private static ThreadLocal<IStopwatch> s_clock = new ThreadLocal<IStopwatch>();

        private static SchedulerQueue<TimeSpan> GetQueue() => s_threadLocalQueue.Value;

        private static void SetQueue(SchedulerQueue<TimeSpan> newQueue)
        {
            s_threadLocalQueue.Value = newQueue;
        }
        ...
        private static TimeSpan Time
        {
            get
            {
                if (s_clock.Value == null)
                    s_clock.Value = ConcurrencyAbstractionLayer.Current.StartStopwatch();

                return s_clock.Value.Elapsed;
            }
        }
